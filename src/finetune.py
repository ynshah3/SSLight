...
parser.add_argument('--data', type=Path, default='./imagenet/',
                    help='path to dataset')
parser.add_argument('--pretrained', type=str, default='./unsup_logs/checkpoints',
                    help='path to pretrained model')
parser.add_argument('--weights', default='finetune', type=str,
                    choices=('finetune', 'freeze'),
                    help='finetune or freeze resnet weights')
parser.add_argument('--train-percent', default=1, type=int,
                    choices=(100, 10, 1),
...
parser.add_argument('--checkpoint-dir', default='./checkpoints/lincls/', type=Path,
...
def main_worker(gpu, args):
    for epoch_iter in range(40):
        ...
        if args.rank == 0:
            ...
            stats_file = open(args.checkpoint_dir / f'stats_{epoch_iter}.txt', 'a', buffering=1)
            print(' '.join(sys.argv))
        ...
        model = models.resnet18().cuda(gpu)
        state_dict = torch.load(args.pretrained + f'/DINO_resnet18_{epoch_iter}.pth.tar', map_location='cpu')
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if args.rank == 0:
                ...
                torch.save(state, args.checkpoint_dir / f'checkpoint_{epoch_iter}.pth')
